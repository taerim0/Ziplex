...
![패킹: 프로젝트 가이드/룰/파일별 요약 검토 화면](docs/images/gui-pack-review.png)

분석이 끝나면 저장 전에 검토 단계에서 멈춥니다 — 프로젝트 이름, 가이드, 룰, 파일별 요약을 고칠 수 있고(CLI와 같은 기준으로 신뢰도가 낮은 것만 검토 대상으로 표시됩니다).
