...
def get_folders(aif_path: str) -> dict:
    ...
    for every folder that directly contains at least one collected file --
    the aggregate-level counterpart to list_files()'s per-file summaries,
    generated by folder_summary.py during pack(). A root-level file's
    folder is "." (Path.parent's own natural value, not a special-cased
    sentinel).

    `file_count` (added later than `summary`/`confidence`) is how many
    files that one summary sentence is actually standing in for -- a real
    gap found dogfooding Ziplex on its own repo: a high-fan-out folder
    (src/ziplex/ itself, 49 files) gets exactly the same one generic
    sentence a 2-file folder does, with nothing in the response to tell a
    caller it's a much coarser rollup. A caller weighing how much to trust
    one folder's summary can check this before deciding whether to drill
    into list_files(folder=...) instead.

    A project packed before `folders` existed at all returns {} rather
    than raising -- same backward-compat default `tech_stack`'s own
    section in skill_export.py already uses for the same reason. A project
    packed before `file_count` existed (but after `folders` itself) simply
    has entries without that key -- callers should treat a missing
    `file_count` as unknown, not zero.
    """
    aif = _load_json(aif_path)
