...
def test_load_config_ignores_a_non_list_value_and_keeps_the_default(tmp_path):
    # a plausible typo -- a bare string instead of a one-item list -- must
    # not survive into collection_kwargs(), which concatenates it onto a
    # list and would otherwise raise TypeError on every later command.
    (tmp_path / CONFIG_FILENAME).write_text(json.dumps({"include": "src/**"}), encoding="utf-8")
...
def test_load_config_ignores_a_list_with_non_string_items(tmp_path):
    (tmp_path / CONFIG_FILENAME).write_text(json.dumps({"ignore": ["ok", 1]}), encoding="utf-8")
